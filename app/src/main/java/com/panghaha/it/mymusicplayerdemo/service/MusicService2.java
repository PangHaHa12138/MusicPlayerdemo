...
public class MusicService2 extends Service implements MediaPlayer.OnCompletionListener {
    ...
    private void initMusicDatas(Intent intent) {
        ...
        List<MusicData2> musicDatas = (List<MusicData2>) intent.getSerializableExtra(MusicPlayActivity.PARAM_MUSIC_LIST);
        mMusicDatas.clear();
...
//            mMediaPlayer = MediaPlayer.create(getApplicationContext(), mMusicDatas.get(index).getMusicRes());
            Uri path = Uri.parse(mMusicDatas.get(index).getMusicRes());
            mMediaPlayer = MediaPlayer.create(getApplicationContext(), path);
